# Visualling Quantum Wave Function ( Finite Well ) ________________________________
#Libraries
import numpy as np
import cmath
...
for j in range(int(0.4*m)):
	psi[0,j]=exp(-600*(x[j]-0.35)**2)*exp(-J*50*x[j])
	V[j]=0
for j in range(int(0.4*m),int(0.6*m)):
	psi[0,j]=exp(-600*(x[j]-0.35)**2)*exp(-J*50*x[j])
	V[j]=-800
for j in range(int(0.6*m),m):
	psi[0,j]=exp(-600*(x[j]-0.35)**2)*exp(-J*50*x[j])
	V[j]=0
